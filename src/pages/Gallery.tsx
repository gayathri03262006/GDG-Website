import React, { useState, useEffect } from 'react';
import { motion } from 'framer-motion';
import HeroSection from '../components/HeroSection';
import { galleryImages, eventGroups } from '../data/galleryData';

const Gallery: React.FC = () => {
  const [currentEventIndex, setCurrentEventIndex] = useState(0);
  const [currentImageIndex, setCurrentImageIndex] = useState(0);

  // Auto-cycle through events and images
  useEffect(() => {
    const eventTimer = setInterval(() => {
      setCurrentEventIndex((prev) => (prev + 1) % eventGroups.length);
      setCurrentImageIndex(0);
    }, 4000);

    return () => clearInterval(eventTimer);
  }, []);

  useEffect(() => {
    if (eventGroups[currentEventIndex]?.images.length > 1) {
      const imageTimer = setInterval(() => {
        setCurrentImageIndex((prev) => (prev + 1) % eventGroups[currentEventIndex].images.length);
      }, 2000);

      return () => clearInterval(imageTimer);
    }
  }, [currentEventIndex]);

  const scrollingImages = [...galleryImages, ...galleryImages]; // Duplicate for seamless loop

  return (
    <div className="min-h-screen">
      <HeroSection
        title="Gallery"
        subtitle="Explore moments captured from our events, workshops, and community activities."
        backgroundImage="/gallery_hero.svg"
      />

      {/* Infinite Scrolling Gallery */}
      <section className="py-16 px-4 bg-gray-50 dark:bg-gray-900 overflow-hidden">
        <div className="max-w-7xl mx-auto">
          <motion.div
            initial={{ y: 50, opacity: 0 }}
            whileInView={{ y: 0, opacity: 1 }}
            transition={{ duration: 0.8 }}
            viewport={{ once: true }}
            className="text-center mb-12"
          >
            <h2 className="font-heading text-3xl md:text-4xl font-bold text-gray-900 dark:text-white mb-6">
              Event Highlights
            </h2>
            <p className="font-body text-xl text-gray-600 dark:text-gray-400 max-w-3xl mx-auto">
              A visual journey through our community events and activities.
            </p>
          </motion.div>

          {/* Row 1 - Left to Right */}
          <div className="mb-8 overflow-hidden">
            <motion.div
              animate={{ x: ['0%', '-50%'] }}
              transition={{ 
                duration: 30, 
                repeat: Infinity, 
                ease: 'linear' 
              }}
              className="flex space-x-6"
            >
              {scrollingImages.map((image, index) => (
                <div
                  key={`row1-${index}`}
                  className="flex-shrink-0 w-80 h-60 rounded-lg overflow-hidden shadow-lg"
                >
                  <img
                    src={image.url}
                    alt={image.alt}
                    className="w-full h-full object-cover hover:scale-105 transition-transform duration-300"
                  />
                </div>
              ))}
            </motion.div>
          </div>

          {/* Row 2 - Right to Left */}
          <div className="mb-8 overflow-hidden">
            <motion.div
              animate={{ x: ['-50%', '0%'] }}
              transition={{ 
                duration: 35, 
                repeat: Infinity, 
                ease: 'linear' 
              }}
              className="flex space-x-6"
            >
              {scrollingImages.map((image, index) => (
                <div
                  key={`row2-${index}`}
                  className="flex-shrink-0 w-80 h-60 rounded-lg overflow-hidden shadow-lg"
                >
                  <img
                    src={image.url}
                    alt={image.alt}
                    className="w-full h-full object-cover hover:scale-105 transition-transform duration-300"
                  />
                </div>
              ))}
            </motion.div>
          </div>

          
        </div>
      </section>

      {/* Event-wise Image Cycle */}
      <section className="py-16 px-4">
        <div className="max-w-7xl mx-auto">
          <motion.div
            initial={{ y: 50, opacity: 0 }}
            whileInView={{ y: 0, opacity: 1 }}
            transition={{ duration: 0.8 }}
            viewport={{ once: true }}
            className="text-center mb-12"
          >
            <h2 className="text-3xl md:text-4xl font-bold text-gray-900 dark:text-white mb-6">
              Featured Events
            </h2>
            <p className="text-xl text-gray-600 dark:text-gray-400 max-w-3xl mx-auto">
              Experience our events through these carefully curated moments.
            </p>
          </motion.div>

          <div className="flex justify-center">
            <div className="relative w-full max-w-4xl">
              {/* Image Stack */}
              <div className="relative h-96 rounded-lg overflow-hidden shadow-2xl">
                {eventGroups.length > 0 && eventGroups[currentEventIndex] && (
                  <motion.div
                    key={`${currentEventIndex}-${currentImageIndex}`}
                    initial={{ opacity: 0, scale: 1.1 }}
                    animate={{ opacity: 1, scale: 1 }}
                    exit={{ opacity: 0, scale: 0.9 }}
                    transition={{ duration: 0.8 }}
                    className="absolute inset-0"
                  >
                    <img
                      src={eventGroups[currentEventIndex].images[currentImageIndex]?.url}
                      alt={eventGroups[currentEventIndex].images[currentImageIndex]?.alt}
                      className="w-full h-full object-cover"
                    />
                    <div className="absolute inset-0 bg-gradient-to-t from-black/50 to-transparent" />
                    <div className="absolute bottom-6 left-6 right-6 text-white">
                      <h3 className="text-2xl font-bold mb-2">
                        {eventGroups[currentEventIndex].event}
                      </h3>
                      <p className="text-lg opacity-90">
                        {eventGroups[currentEventIndex].images[currentImageIndex]?.alt}
                      </p>
                    </div>
                  </motion.div>
                )}
              </div>

              {/* Event Navigation Dots */}
              <div className="flex justify-center mt-6 space-x-2">
                {eventGroups.map((_, index) => (
                  <button
                    key={index}
                    onClick={() => {
                      setCurrentEventIndex(index);
                      setCurrentImageIndex(0);
                    }}
                    className={`w-3 h-3 rounded-full transition-colors duration-200 ${
                      index === currentEventIndex
                        ? 'bg-blue-600'
                        : 'bg-gray-300 hover:bg-gray-400'
                    }`}
                  />
                ))}
              </div>

              {/* Image Navigation */}
              {eventGroups[currentEventIndex] && eventGroups[currentEventIndex].images.length > 1 && (
                <div className="flex justify-center mt-4 space-x-1">
                  {eventGroups[currentEventIndex].images.map((_, index) => (
                    <button
                      key={index}
                      onClick={() => setCurrentImageIndex(index)}
                      className={`w-2 h-2 rounded-full transition-colors duration-200 ${
                        index === currentImageIndex
                          ? 'bg-blue-400'
                          : 'bg-gray-200 hover:bg-gray-300'
                      }`}
                    />
                  ))}
                </div>
              )}
            </div>
          </div>
        </div>
      </section>
    </div>
  );
};

export default Gallery;